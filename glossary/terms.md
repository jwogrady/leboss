...
The physical location — including the legal jurisdiction — where data is stored and processed. LEBOSS requires that governing entities be informed of data residency and retain the right to enforce residency requirements where legally or operationally necessary.

---

## Data Sovereignty

The practical control a governing entity exercises over their operational data, determined not only by legal right but by architectural reality — specifically, by the topology of the systems that store, process, and grant access to that data.
...
Under LEBOSS, service providers may collect limited derived data (such as operational telemetry) under disclosed and consented conditions, but may not use derived data to benefit third parties or to build products sold to others without explicit authorization from the governing entity.
...
The body of data that is materially required for the operation, continuity, accountability, or reconstruction of a governed business environment. Primary operational data is owned by the governing entity and subject to the full data ownership doctrine of the LEBOSS standard.

The boundary of primary operational data is functional, not categorical. Data labeled as auxiliary, metadata, configuration, or supplementary is still primary operational data if it is materially required for continuity, accountability, or reconstruction of the governed environment. A conformant system MUST NOT narrow the scope of primary operational data in a way that excludes materially required operational content from ownership, audit, or portability obligations.

Illustrative examples include customer records, transaction history, appointment records, communications between the business and its customers, workflow state and operational configuration when materially required for reconstruction, inventory records, and employee records. These examples do not define the boundary — the governing criterion is functional role.

Normative rules: LEBOSS-OWN-1, LEBOSS-OWN-9, LEBOSS-OWN-10.
